  ...
  return (
    <div className="detailPage">
      ...
      <section className={`characterHero element-${character.elementCode}`}>
        {character.backgroundPath.startsWith('/cache/') && (
          <img className="heroBackground" src={character.backgroundPath} alt="" />
        )}
        <div className="heroArt" aria-hidden="true">
          {character.iconPath.startsWith('/cache/') ? (
            <img src={character.iconPath} alt="" />
          ) : (
        ...
        <div className="heroContent">
          ...
          <div className="heroTags">
            <span>
              {'◆'.repeat(character.rarity)} <small>{character.rarity} ESTRELAS</small>
            </span>
            {profile.region && <span>{profile.region}</span>}
            {profile.faction && <span>{profile.faction}</span>}
          </div>
          <p className="heroDescription">
            {profile.description || 'Detalhes ainda não sincronizados para este personagem.'}
          </p>
          <div className="heroActions">
            <button onClick={openAccountEditor}>
              {character.owned ? 'Editar minha conta' : 'Registrar na conta'}
            </button>
        ...
        <aside className="heroMeta">
          ...
          <strong>
            {character.owned ? `Possuído · S${character.sequence}` : 'Não registrado'}
          </strong>
          {character.owned && (
            <>
              <span>NÍVEL</span>
      ...
      <nav className="detailTabs" aria-label="Seções do personagem">
        ...
        <button
          className={tab === 'kit' || tab === 'forte' || tab === 'tree' ? 'active' : ''}
          onClick={() => setTab('kit')}
        >
          Kit & Árvore{' '}
          <span>{profile.skills.length + (profile.extras?.skillTree?.length || 0)}</span>
        </button>
        <button className={tab === 'chains' ? 'active' : ''} onClick={() => setTab('chains')}>
          Sequências <span>{profile.chains.length}</span>
        </button>
        <button className={tab === 'materials' ? 'active' : ''} onClick={() => setTab('materials')}>
          Materiais <span>{profile.progression?.ascensions?.length || 0}</span>
        </button>
        <button className={tab === 'stats' ? 'active' : ''} onClick={() => setTab('stats')}>
          Atributos
        </button>
        <button className={tab === 'lore' ? 'active' : ''} onClick={() => setTab('lore')}>
          Lore
        </button>
      </nav>
      ...
      {accountDraft && (
        <div className="modalBackdrop" onMouseDown={() => setAccountDraft(undefined)}>
          <form
          ...
          >
            <div className="modalHeader">
              <div>
                <span className="sectionLabel">CONTA LOCAL</span>
                <h2 id="account-title">{character.name}</h2>
              </div>
              <button type="button" onClick={() => setAccountDraft(undefined)} aria-label="Fechar">
                ×
              </button>
            </div>
            <label className="ownershipToggle">
              <input
                type="checkbox"
                checked={accountDraft.owned}
                onChange={(event) =>
                  setAccountDraft({ ...accountDraft, owned: event.target.checked })
                }
              />
              <span>
                <strong>Personagem possuído</strong>
                <small>Inclui o personagem em builds e alternativas da conta.</small>
              </span>
            </label>
            <div className="accountFields">
              <label>
                ...
                <input
                  ...
                  min={1}
                  max={90}
                  disabled={!accountDraft.owned}
                  value={accountDraft.level}
                  onChange={(event) =>
                    setAccountDraft({ ...accountDraft, level: Number(event.target.value) })
                  }
                />
              </label>
              <label>
                ...
                <select
                  disabled={!accountDraft.owned}
                  value={accountDraft.sequence}
                  onChange={(event) =>
                    setAccountDraft({ ...accountDraft, sequence: Number(event.target.value) })
                  }
                >
            ...
            <label className="ownershipToggle compact">
              <input
                type="checkbox"
                checked={accountDraft.favorite}
                onChange={(event) =>
                  setAccountDraft({ ...accountDraft, favorite: event.target.checked })
                }
              />
              <span>
                <strong>Favorito</strong>
                <small>Pode ser favorito mesmo sem estar possuído.</small>
              </span>
            </label>
            <div className="modalActions">
              <button type="button" onClick={() => setAccountDraft(undefined)}>
                Cancelar
              </button>
              <button className="primaryButton" disabled={savingAccount}>
                {savingAccount ? 'Salvando…' : 'Salvar na conta'}
              </button>
            </div>
          </form>
        </div>
...
export function Materials({ profile }: { profile: CharacterProfile }) {
  const skills = useMemo(
    () => (profile.progression?.skills || []).filter((skill) => skill.maxLevel > 1),
    [profile]
  );
  const [currentLevel, setCurrentLevel] = useState(1);
  const [targetLevel, setTargetLevel] = useState(90);
  const [currentSkills, setCurrentSkills] = useState<Record<string, number>>(() =>
    Object.fromEntries(skills.map((skill) => [skill.nodeId, 1]))
  );
  const [targetSkills, setTargetSkills] = useState<Record<string, number>>(() =>
    Object.fromEntries(skills.map((skill) => [skill.nodeId, skill.maxLevel]))
  );
  ...
  async function calculate() {
    ...
    try {
      setPlan(
        await calculateCharacterProgression({
  ...
  if (!profile.progression?.ascensions?.length)
    return <DetailEmpty text="Materiais ainda não sincronizados." />;
  return (
    <div className="materialsPlanner">
      <header className="materialsTitle">
        <div>
          <span className="sectionLabel">DADOS OFICIAIS · {profile.character.gameVersion}</span>
          <h2>Materiais de ascensão</h2>
          <p>
            Nv.{currentLevel} → Nv.{targetLevel}
          </p>
        </div>
        {calculating && <span className="calculatingBadge">Atualizando…</span>}
      </header>
      <MaterialCards
        costs={plan?.ascensions || []}
        emptyText="Nenhum material de ascensão neste intervalo."
      />
      ...
      <section className="levelPlanner">
        ...
        <label>
          <span>De {currentLevel}</span>
          <input
            type="range"
            min={1}
            max={90}
            value={currentLevel}
            onChange={(event) => setCurrentLevel(Math.min(Number(event.target.value), targetLevel))}
          />
        </label>
        <label>
          <span>Até {targetLevel}</span>
          <input
            type="range"
            min={1}
            max={90}
            value={targetLevel}
            onChange={(event) => setTargetLevel(Math.max(Number(event.target.value), currentLevel))}
          />
        </label>
        <div className="levelTicks">
          {[1, 20, 30, 40, 50, 60, 70, 80, 90].map((level, index, values) => (
            <span
              className={index === 0 ? 'first' : index === values.length - 1 ? 'last' : ''}
              style={{ left: `${((level - 1) / 89) * 100}%` }}
              key={level}
            >
              {level}
            </span>
      ...
      <section className="skillMaterialSection">
        <header>
          <div>
            <h2>Materiais de habilidade</h2>
            <p>Forte e nós selecionados</p>
          </div>
          <label className="check">
            <input
              type="checkbox"
              checked={includeUnlocks}
              onChange={(event) => setIncludeUnlocks(event.target.checked)}
            />
            Incluir nós passivos
          </label>
        </header>
        <MaterialCards
          costs={plan?.skills || []}
          emptyText="Nenhum material de habilidade neste intervalo."
        />
        <div className="skillLevelGrid">
          {skills.map((skill) => (
            <article key={skill.nodeId}>
              <div>
              ...
              <label>
                <span>De</span>
                <select
                  value={currentSkills[skill.nodeId]}
                  onChange={(event) =>
                    setCurrentSkills({
                      ...currentSkills,
                      [skill.nodeId]: Number(event.target.value),
                    })
                  }
                >
                  {levels(skill.maxLevel).map((level) => (
              ...
              <label>
                <span>Até</span>
                <select
                  value={targetSkills[skill.nodeId]}
                  onChange={(event) =>
                    setTargetSkills({ ...targetSkills, [skill.nodeId]: Number(event.target.value) })
                  }
                >
                  {levels(skill.maxLevel).map((level) => (
                    <option key={level}>{level}</option>
      ...
      {plan && (
        <footer className="materialsTotal">
          <span>Total combinado</span>
          <strong>{plan.total.length} tipos de material</strong>
        </footer>
      )}
      {error && (
        <p className="progressionError" role="alert">
          {error}
        </p>
      )}
    </div>
...
function MaterialCards({ costs, emptyText }: { costs: MaterialCost[]; emptyText: string }) {
  ...
  return (
    <div className="materialCards">
      {costs.map((cost) => (
        <article
          className={`materialTile rarity-${cost.material.rarity}`}
          key={cost.material.id}
          title={cost.material.description}
        >
          <div>
            {cost.material.iconPath?.startsWith('/cache/') ? (
              <img src={cost.material.iconPath} alt="" />
            ) : (
              <span>◇</span>
            )}
          </div>
...
function Overview({ profile }: { profile: CharacterProfile }) {
  ...
  return (
    <div className="overviewGrid">
      {!!profile.extras?.tags?.length && (
        <section className="detailPanel roleTagsPanel">
          <span className="sectionLabel">FUNÇÕES OFICIAIS</span>
          <div className="roleTagGrid">
            {profile.extras.tags.map((tag) => (
              <article
                key={tag.id}
                style={{ '--tag-color': `#${tag.color || '67dce3'}` } as CSSProperties}
              >
                <i />
                <div>
      ...
      <section className="detailPanel">
        ...
        <p>
          {profile.talentDescription ||
            profile.description ||
            'Os detalhes serão preenchidos na próxima sincronização.'}
        </p>
      </section>
      <section className="detailPanel weaponPanel">
        ...
        {profile.signatureWeapon?.name ? (
          <>
            <div className="weaponHeading">
              <div className="weaponGlyph" aria-hidden="true">
                {profile.signatureWeapon.iconPath.startsWith('/cache/') ? (
                  <img src={profile.signatureWeapon.iconPath} alt="" />
                ) : (
                  '◇'
                )}
              </div>
              <div>
                <h2>{profile.signatureWeapon.name}</h2>
                <small>
                  {profile.signatureWeapon.type} · {'◆'.repeat(profile.signatureWeapon.rarity)}
                </small>
              </div>
            </div>
          ...
          <p>
            {rover
              ? 'Nenhuma arma recomendada foi encontrada nos dados sincronizados.'
              : 'Nenhuma arma assinatura foi encontrada nos dados sincronizados.'}
          </p>
        )}
      </section>
      <section className="detailPanel quickKit">
        ...
        {profile.skills.slice(0, 6).map((skill) => (
          <div key={skill.nodeId}>
            <i />
            <span>
              <strong>{skill.type || 'Habilidade'}</strong>
...
function LegacyUnifiedKitTree({ profile }: { profile: CharacterProfile }) {
  ...
  const byID = useMemo(
    () => new Map(profile.skills.map((skill) => [skill.nodeId, skill])),
    [profile.skills]
  );
  ...
  const selectedSkill =
    byID.get(selectedNodeId) || profile.skills.find((s) => s.nodeId === selectedNodeId);
  const progression = profile.progression?.skills?.find((entry) => entry.nodeId === selectedNodeId);
  const isForteCircuit =
    selectedNode?.nodeType === 1 ||
    selectedSkill?.name.toLowerCase().includes('forte') ||
    selectedSkill?.type?.toLowerCase().includes('forte');

  return (
    <div className="unifiedKitTree">
      {!!branches.length && (
        <section className="branchSection" style={{ marginTop: 0, marginBottom: 24 }}>
          ...
          <div>
            {branches.map((branch) => (
              <article key={branch.id}>
                ...
                <h3>{branch.name}</h3>
                <FormattedText text={branch.description} />
              </article>
            ))}
          </div>
        </section>
      ...
      <section className="treePage" style={{ marginTop: 12 }}>
        <header>
          ...
          <span>
            {nodes.length || profile.skills.length} nós · Clique em qualquer quadrado para
            inspecionar
          </span>
        </header>
        <div className="skillTreeGrid">
          {nodes.map((node) => {
            ...
            return (
              ...
              >
                <div
                  className="treeNodeIndex"
                  style={{
                    borderColor: isSelected ? 'var(--cyan)' : undefined,
                    background: isSelected ? 'var(--cyan)' : undefined,
                    color: isSelected ? '#090e13' : undefined,
                ...
                <div>
                  ...
                  <strong style={{ color: isSelected ? 'var(--cyan)' : undefined }}>
                    {skill?.name || `Nó ${node.nodeId}`}
                  </strong>
                  {!!node.parentNodes?.length && (
                    <span>Depende de {node.parentNodes.join(', ')}</span>
                  )}
                </div>
      ...
      {selectedSkill ? (
          ...
          >
            <div>
              <span className="sectionLabel" style={{ color: 'var(--cyan)' }}>
                INSPETOR DE MAESTRIA · NÓ SELECIONADO #{selectedNodeId}
              </span>
              <h2 style={{ fontSize: 20, margin: '4px 0 0 0' }}>{selectedSkill.name}</h2>
            </div>
            <small style={{ color: 'var(--muted)', font: '11px Consolas, monospace' }}>
              {selectedSkill.type || nodeTypeName(selectedNode?.nodeType || 2)}
            </small>
          </header>

          <article
            className={openMultiplier ? 'skillCard expanded' : 'skillCard'}
            style={{ borderColor: 'rgba(103, 220, 227, 0.4)', background: 'rgba(14, 19, 27, 0.7)' }}
          >
            <div
              className="skillIndex"
            ...
            <div>
              ...
              {isForteCircuit && forte && (forte.actions?.length || 0) > 0 && (
                <div
                  className="forteActionsInline"
                  style={{ marginTop: 18, paddingTop: 16, borderTop: '1px solid var(--border)' }}
                >
                  <span className="sectionLabel" style={{ display: 'block', marginBottom: 12 }}>
                    ENTRADAS E COMBOS DE ROTAÇÃO (GUIA DO FORTE)
                  </span>
                  <div className="forteActions" style={{ gap: 8 }}>
                    {(forte.actions || []).map((action, idx) => (
                      <article key={`${action.name}-${idx}`} style={{ padding: 12 }}>
                        <header>
                          <span>{action.name || `Etapa ${idx + 1}`}</span>
                          <div>
                            {(action.inputs || []).map((input) => (
                              <kbd key={input}>{input}</kbd>
                            ))}
                          </div>
                        </header>
            ...
            {openMultiplier && progression && (
              <div className="skillValues">
                <div className="skillValueHeader">
                  <span>Escala oficial</span>
                  {levels(progression.maxLevel).map((level) => (
                    <b key={level}>Nv. {level}</b>
                  ))}
                </div>
                {progression.values.map((row) => (
                  <div className="skillValueRow" key={row.name}>
                    ...
                    {levels(progression.maxLevel).map((level) => (
                      <span key={level}>{row.values[level - 1] || '—'}</span>
                    ))}
                  </div>
                ))}
              </div>
        ...
        <div className="emptyState detailEmpty" style={{ marginTop: 28, padding: 32 }}>
          ...
          <p>
            Clique em qualquer quadrado do grid da árvore para visualizar a descrição completa e a
...
function ResonanceKitTree({ profile }: { profile: CharacterProfile }) {
  ...
  const defaultSkill =
    profile.skills.find((skill) => byNodeID.get(skill.nodeId)?.nodeType === 1) || profile.skills[0];
  ...
  const visibleSkills = profile.skills.filter((skill) => {
    ...
    const matchesBranch =
      !selectedBranchID || !node?.branchIds.length || node.branchIds.includes(selectedBranchID);
    const matchesSearch =
      !normalizedQuery ||
      `${skill.name} ${skill.type} ${skill.nodeId}`.toLocaleLowerCase().includes(normalizedQuery);
  ...
  return (
    <div className="nanokaKit">
      ...
      {!!branches.length && (
        <div className="nanokaKitModes" role="tablist" aria-label="Modos de ressonância">
          {branches.map((branch) => (
            <button
              role="tab"
      ...
      <section className="nanokaProgression" aria-labelledby="skill-progression-title">
        <header>
          <div>
            <span className="sectionLabel">COMBAT SKILLS</span>
            <h3 id="skill-progression-title">Níveis do Kit</h3>
          </div>
        ...
        <div className="nanokaProgressionList">
          {visibleSkills.map((skill) => {
            ...
            return (
              <article key={skill.nodeId}>
                <button
                  className="nanokaProgressionIdentity"
                  onClick={() => setExpandedSkillID(skill.nodeId)}
                >
                ...
                {maxLevel > 1 && (
                  <label className="nanokaLevelControl">
                ...
                {!!bonuses.length && (
                  <div className="nanokaBonusChips" aria-label={`Bônus de ${skill.name}`}>
                    <span>STAT BONUS</span>
                    <div>
                      {bonuses.map((bonus) => (
                        <button key={bonus.nodeId} onClick={() => setExpandedSkillID(bonus.nodeId)}>
                          <small>{bonus.nodeId}</small>
          ...
          {!visibleSkills.length && (
            <p className="nanokaNoResults">Nenhuma habilidade corresponde aos filtros atuais.</p>
          )}
        </div>
      </section>
      ...
      <section className="nanokaSkillLibrary" aria-labelledby="combat-skills-title">
        <header>
          <div>
            <span className="sectionLabel">DETALHES SINCRONIZADOS</span>
            <h3 id="combat-skills-title">Habilidades de combate</h3>
          </div>
        ...
        <div className="nanokaSkillGrid">
          {visibleSkills.map((skill) => {
            ...
            const bonuses = bonusesFor(skill.nodeId);
            const isForte =
              node?.nodeType === 1 || skill.type.toLocaleLowerCase().includes('forte');
            return (
              <article
                className={`nanokaSkillCard${expanded ? ' expanded' : ''}`}
                key={skill.nodeId}
              >
                <button
                  className="nanokaSkillCardHeader"
                  onClick={() => setExpandedSkillID(expanded ? '' : skill.nodeId)}
                ...
                {expanded && (
                  <div className="nanokaSkillCardBody">
                    <section>
                      <span className="sectionLabel">DESCRIÇÃO OFICIAL</span>
                      {skill.description ? (
                        <FormattedText text={skill.description} />
                      ) : (
                        <p>Não disponível nos dados sincronizados.</p>
                      )}
                    </section>

                    {!!bonuses.length && (
                      <section className="nanokaCardBonuses">
                        <span className="sectionLabel">STAT BONUS</span>
                        <div>
                          {bonuses.map((bonus) => (
                            <article key={bonus.nodeId}>
                              <SkillNodeIcon type={bonus.type} nodeType={4} size={19} />
                              <span>
                                <strong>{bonus.name}</strong>
                                <small>{bonus.description || 'Valor não disponível.'}</small>
                              </span>
                            </article>
                          ))}
                        </div>
                    ...
                    <section className="nanokaOfficialValues">
                      <span className="sectionLabel">
                        SKILL ATTRIBUTES{maxLevel > 1 ? ` · NV.${level}` : ''}
                      </span>
                      {progression?.values.length ? (
                        <div>
                          {progression.values.map((row) => (
                            <article key={row.name}>
                              <span>{row.name}</span>
                              <strong>{row.values[level - 1] || 'Não disponível'}</strong>
                            </article>
                          ))}
                        </div>
                      ) : (
                        <p>Valores numéricos não disponíveis nos dados sincronizados.</p>
                      )}
                    </section>

                    {!!node?.branchIds.length && (
                      <section className="nanokaLinkedModes">
                        <span className="sectionLabel">MODOS VINCULADOS</span>
                        <div>
                          {node.branchIds.map((branchID) => {
                            const branch = branches.find((item) => item.id === branchID);
                            return (
                              <button key={branchID} onClick={() => setSelectedBranchID(branchID)}>
                                <small>#{branchID}</small>
                                <strong>{branch?.name || 'Não disponível'}</strong>
                              </button>
                            );
                          })}
                        </div>
                    ...
                    {isForte && forte && forte.actions.length > 0 && (
                      <section className="nanokaForteActions">
                        <span className="sectionLabel">SKILL INPUT</span>
                        <div>
                          {forte.actions.map((action, index) => (
                            <article key={`${action.name}-${index}`}>
                              <header>
                                <strong>{action.name || `Etapa ${index + 1}`}</strong>
                                <div>
                                  {action.inputs.map((input) => (
                                    <kbd key={input}>{input}</kbd>
                                  ))}
...
function LegacyResonanceKitTree({ profile }: { profile: CharacterProfile }) {
  ...
  const defaultID =
    nodes.find((node) => node.nodeType === 1)?.nodeId ||
    nodes[0]?.nodeId ||
    profile.skills[0]?.nodeId ||
    '1';
  ...
  const visualNodes = nodes.length
    ? nodes
  ...
  const filteredNodes = visualNodes.filter((node) => {
    ...
    return (
      !normalizedQuery ||
      `${node.nodeId} ${skill?.name || ''} ${skill?.type || nodeTypeName(node.nodeType)}`
        .toLocaleLowerCase()
  ...
  return (
    <div className="resonanceWorkspace resonanceWorkspacePremium">
      <aside className="resonanceNavigator" aria-label="Navegador do Kit e Árvore">
        <header>
          <span className="sectionLabel">NAVEGADOR</span>
          ...
          {filteredNodes.map((node) => {
            ...
            return (
              ...
              >
                ...
                {!!node.branchIds.length && (
                  <b>
                    {node.branchIds.length} {node.branchIds.length === 1 ? 'modo' : 'modos'}
                  </b>
                )}
              </button>
            );
        ...
        <footer>
          <span>
            <i className="ringKey active" />
            Ativa
          </span>
          <span>
            <i className="ringKey passive" />
            Passiva
          </span>
          <span>
            <i className="ringKey attribute" />
            Atributo
          </span>
        </footer>
      </aside>
      ...
      <section className="resonanceLensPanel">
        {!!branches.length && (
          <div className="resonanceModes" role="tablist" aria-label="Modos de ressonância">
            {branches.map((branch) => (
              <button
                role="tab"
                aria-selected={branch.id === selectedBranchID}
        ...
        <header className="resonanceLensHeader">
          <div>
            <span className="sectionLabel">MAPA DE RESSONÂNCIA</span>
            <h2>Kit e dependências</h2>
          </div>
        ...
        <div className="resonanceLens" aria-label="Mapa radial de habilidades">
          ...
          <div className="lensCore" aria-hidden="true">
            <GitBranch size={22} />
          </div>
          {ringGroups.map((ring, ringIndex) =>
            ring.map((node, index) => {
              const skill = byID.get(node.nodeId);
              const branchMatch =
                !selectedBranchID ||
                !node.branchIds.length ||
                node.branchIds.includes(selectedBranchID);
        ...
        {selectedSkill ? (
          <>
            <header>
              ...
              {!!selectedNode?.branchIds.length && (
                <b>
                  {selectedNode.branchIds.length}{' '}
                  {selectedNode.branchIds.length === 1 ? 'modo' : 'modos'}
                </b>
              )}
            </header>
              ...
              <div>
                {parentNodes.length ? (
                  parentNodes.map((parent) => (
                    <button key={parent.id} onClick={() => selectNode(parent.id)}>
                      <span>{parent.id}</span>
                      <strong>{parent.name || `Nó ${parent.id}`}</strong>
                      <ChevronRight size={14} />
                    </button>
                  ))
                ) : (
                  <p>Sem dependência registrada para este nó.</p>
                )}
                {!!parentNodes.length && (
                  <div className="currentDependency">
                    <span>{selectedNodeID}</span>
            ...
            {!!selectedNode?.branchIds.length && (
              <section className="nodeModes">
                <span className="sectionLabel">MODOS VINCULADOS</span>
                <div>
                  {selectedNode.branchIds.map((branchID) => {
                    const branch = branches.find((item) => item.id === branchID);
                    return (
                      <button
                      ...
                      >
                        <span>#{branchID}</span>
                        <strong>{branch?.name || `Modo ${branchID}`}</strong>
                      </button>
                    );
                  })}
                </div>
            ...
            {maxLevel > 1 && (
              <section className="inspectorLevels">
                <span className="sectionLabel">NÍVEL</span>
                <div>
                  {levels(maxLevel).map((value) => (
                    <button
            ...
            <section className="officialValues">
              <header>
                <span className="sectionLabel">
                  VALORES OFICIAIS{maxLevel > 1 ? ` · NV. ${level}` : ''}
                </span>
            ...
            {isForteCircuit && forte && forte.actions.length > 0 && (
              <section className="inspectorForte">
          ...
          <div className="inspectorEmpty">
            <GitBranch size={30} />
            <strong>Nó não disponível</strong>
            <p>Selecione um nó com dados sincronizados.</p>
          </div>
        )}
      </aside>
    </div>
...
function ForteMeridianKitTree({ profile }: { profile: CharacterProfile }) {
  ...
  const rootChains = roots.map((root) => {
    ...
    while (chain.length < 4) {
      const next = visualNodes
        .filter(
          (node) => node.parentNodes.includes(Number(current.nodeId)) && !seen.has(node.nodeId)
        )
  ...
  const modeNodes = visualNodes.filter(
    (node) => !placedIDs.has(node.nodeId) && node.branchIds.length > 0
  );
  ...
  function matchesQuery(nodeID: string) {
    ...
    return `${nodeID} ${skill?.name || ''} ${skill?.type || ''}`
      .toLocaleLowerCase()
      .includes(normalizedQuery);
  }

  return (
    <div className="forteMeridian">
          ...
          <>
            <div className="meridianEmblem" aria-hidden="true">
              <SkillNodeIcon
                ...
                nodeType={selectedNode?.nodeType || 0}
                size={38}
              />
            </div>
            <span className="meridianType">
              {selectedSkill.type || nodeTypeName(selectedNode?.nodeType || 0)}
            </span>
            <h2>{selectedSkill.name}</h2>
      ...
      <section className="meridianCanvas" aria-label="Árvore de habilidades">
        ...
        <div className="meridianForest">
          {rootChains.map((chain, columnIndex) => (
            <div
        ...
        {!!modeNodes.length && (
          <div className="meridianModeNodes" aria-label="Nós vinculados a modos">
            {modeNodes.map((node) => (
              <ForestNode
                key={node.nodeId}
                node={node}
        ...
        {!!remainingNodes.length && (
          <div className="meridianRemaining" aria-label="Outros nós sincronizados">
...
function ForestNode({
  ...
  return (
    <article
      className={`forestNode${selected ? ' selected' : ''}${branchMuted ? ' branchMuted' : ''}${queryMuted ? ' queryMuted' : ''}${connected ? ' connected' : ''}`}
    >
      <button
        onClick={onSelect}
        aria-label={`${skill?.name || `Nó ${node.nodeId}`}${selected ? ', selecionado' : ''}`}
      >
...
function Stats({ profile }: { profile: CharacterProfile }) {
  ...
  const stat = profile.progression?.stats?.find(
    (entry) => entry.ascension === ascension && entry.level === boundedLevel
  );
  const weakness = profile.extras?.weakness;
  if (!profile.progression?.stats?.length)
    return <DetailEmpty text="Atributos ainda não sincronizados." />;
  return (
    <div className="statsPage">
      <section className="statCalculator">
        <header>
          <div>
            <span className="sectionLabel">ATRIBUTOS EXATOS</span>
            <h2>Nível e ascensão</h2>
          </div>
          <strong>
            A{ascension} · Nv.{boundedLevel}
          </strong>
        </header>
        <label>
          Ascensão{' '}
        ...
        <div className="statNumbers">
          <article>
            <small>HP BASE</small>
            <strong>{formatStat(stat?.hp)}</strong>
          </article>
          <article>
            <small>ATK BASE</small>
            <strong>{formatStat(stat?.atk)}</strong>
          </article>
          <article>
            <small>DEF BASE</small>
            <strong>{formatStat(stat?.def)}</strong>
          </article>
        </div>
      </section>
      <section className="weaknessPanel">
        <span className="sectionLabel">FRAQUEZA E QUEBRA</span>
        <h2>Parâmetros internos</h2>
        <div>
          <StatLine label="Acúmulo" value={weakness?.buildUp} />
          <StatLine label="Limite de acúmulo" value={weakness?.buildUpMax} />
          <StatLine label="Bônus total" value={weakness?.totalBonus} percent />
          <StatLine label="Multiplicador de quebra" value={weakness?.breakRatio} percent />
          <StatLine label="Maestria" value={weakness?.mastery} />
        </div>
        <p>
          Valores oficiais da versão {profile.character.gameVersion}. Percentuais são convertidos da
          escala interna de 10.000 pontos.
        </p>
      </section>
    </div>
  );
...
function Lore({ profile }: { profile: CharacterProfile }) {
  ...
  return (
    <div className="lorePage">
      <header>
        <span className="sectionLabel">ARQUIVO PESSOAL</span>
        <h2>Histórias e objetos</h2>
        <p>
          {entries.length} registros oficiais de {profile.character.name}
        </p>
      </header>
      <div>
        {entries.map((entry, index) => (
          <details key={`${entry.title}-${index}`} open={index === 0}>
            <summary>
...
function StatLine({
  ...
  return (
    <div>
      <span>{label}</span>
      <strong>{percent ? `${formatStat(value / 100)}%` : formatStat(value)}</strong>
    </div>
  );
}
  ...
  return (
    <div className="chainTimeline">
      {profile.chains.map((chain) => (
        <article className="chainItem" key={chain.sequence}>
          <div className="chainMarker">
            <span>S{chain.sequence}</span>
          </div>
          <div>
            <span className="sectionLabel">RESONANCE CHAIN {chain.sequence}</span>
            <h2>{chain.name}</h2>
            <FormattedText text={chain.description} />
          </div>
        </article>
      ))}
    </div>
...
function DetailEmpty({ text }: { text: string }) {
  return (
    <div className="emptyState detailEmpty">
      <div className="emptyGlyph">◇</div>
      <h2>{text}</h2>
      <p>Execute uma nova sincronização para buscar os detalhes atuais.</p>
    </div>
  );
}
