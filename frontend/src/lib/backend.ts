...
export async function catalogStatus(): Promise<CatalogStatus> {
  const backend = api();
  return backend
    ? backend.CatalogStatus()
    : import.meta.env.DEV
      ? { count: previewCharacters.length, version: '3.6.1' }
      : emptyStatus;
}
...
export async function calculateCharacterProgression(
  request: ProgressionPlanRequest
...
export async function evaluateBuild(id: number): Promise<BuildEvaluation> {
  const backend = api();
  if (!backend) throw new Error('Theorycraft disponível no desktop.');
...
export async function saveBuff(buff: Buff): Promise<Buff> {
  const backend = api();
  if (!backend) throw new Error('Buffs disponíveis no desktop.');
  return backend.SaveBuff(buff);
}
export async function deleteBuff(id: number): Promise<void> {
  await api()?.DeleteBuff(id);
}
export async function listAIConversations(): Promise<AIConversation[]> {
  return (await api()?.ListAIConversations()) ?? [];
}
export async function assistantChat(request: AssistantRequest): Promise<AIConversation> {
  const backend = api();
  if (!backend) throw new Error('Assistente disponível no desktop.');
  return backend.AssistantChat(request);
}
export async function assistantChatStream(request: AssistantRequest): Promise<AIConversation> {
  const b = api();
  if (!b) throw new Error('Streaming disponível no desktop.');
  return b.AssistantChatStream(request);
}
export async function testAIProvider(request: AIAnalysisRequest) {
  const b = api();
  if (!b) throw new Error('Teste disponível no desktop.');
  return b.TestAIProvider(request);
}
export async function listCharacterGuides(id: number) {
  return (await api()?.ListCharacterGuides(id)) ?? [];
}
export async function listAllCharacterGuides() {
  return (await api()?.ListAllCharacterGuides()) ?? [];
}
export async function syncCharacterGuides(id: number, language = 'en') {
  const b = api();
...
export async function listBuildVersions(id: number) {
  return (await api()?.ListBuildVersions(id)) ?? [];
}
export async function getSettings() {
  const b = api();
  if (!b) throw new Error('Configurações disponíveis no desktop.');
  return b.GetSettings();
}
export async function saveSettings(value: import('../types').AppSettings) {
  const b = api();
  if (!b) throw new Error('Configurações disponíveis no desktop.');
  return b.SaveSettings(value);
}
export async function listDataSourceOptions() {
  const b = api();
  if (!b) throw new Error('Fontes disponíveis no desktop.');
  return b.ListDataSourceOptions();
}
export async function getAccountSummary() {
  const b = api();
  if (!b) throw new Error('Conta disponível no desktop.');
  return b.GetAccountSummary();
}
export async function saveAccountSummary(value: import('../types').AccountSummary) {
  const b = api();
  if (!b) throw new Error('Conta disponível no desktop.');
  return b.SaveAccountSummary(value);
}
export async function listEnemies() {
  return (await api()?.ListEnemies()) ?? [];
}
export async function listFormulaVersions() {
  return (await api()?.ListFormulaVersions()) ?? [];
}
...
export async function exportArchive() {
  const b = api();
  if (!b) throw new Error('Exportação disponível no desktop.');
  return b.ExportArchive();
}
export async function importArchive(payload: string) {
  const b = api();
  if (!b) throw new Error('Importação disponível no desktop.');
  return b.ImportArchive(payload);
}
export async function createManualBackup() {
  const b = api();
  if (!b) throw new Error('Backup disponível no desktop.');
  return b.CreateManualBackup();
}
...
export async function restoreBackup(name: string) {
  const b = api();
  if (!b) throw new Error('Restauração disponível no desktop.');
  return b.RestoreBackup(name);
}
export async function diagnostics() {
  const b = api();
  if (!b) throw new Error('Diagnóstico disponível no desktop.');
