...
const navItems: { id: PageID; label: string }[] = [
  ...
  { id: 'sonata', label: 'Sonata Effects' },
...
export function App() {
  ...
  async function restore() {
    if (
      !window.confirm(
        'Restaurar o snapshot mais recente? O estado atual será salvo antes da restauração.'
      )
    )
      return;
    setLoading(true);
    ...
    <div className="shell">
      ...
      <header className="topbar">
        ...
        >
          ...
          <span>
            <strong>WAVE</strong>ARCHIVE
          </span>
        </button>
        <nav className="globalNav" aria-label="Navegação principal">
          <button
            className={page === 'dashboard' ? 'globalNavLink active' : 'globalNavLink'}
            onClick={() => {
              setPage('dashboard');
              setSelectedID(undefined);
            }}
          ...
          {navGroups.map((group) => (
            <div
              className={`${group.items.includes(page) ? 'navGroup active' : 'navGroup'}${openNav === group.label ? ' open' : ''}`}
              key={group.label}
            >
              <button
                ...
                aria-haspopup="menu"
                onClick={() =>
                  setOpenNav((current) => (current === group.label ? null : group.label))
                }
              >
                {group.label}
        ...
        <div className="topActions">
          ...
          {syncing && (
            <button className="cancelButton" onClick={() => void cancelSync()}>
              Cancelar
            </button>
          )}
          <button className="syncButton" onClick={() => void sync()} disabled={syncing}>
            ...
            {syncing
              ? `${stageLabel(syncProgress?.stage)} ${syncProgress?.progress ?? 0}%`
              : status.version
                ? 'Sincronizado'
                : 'Sincronizar'}
          </button>
      ...
      <main id="main-content" tabIndex={-1}>
        <div className="contextBar">
          ...
          {page === 'characters' && profile && (
            <>
              <ChevronDown size={12} className="contextSeparator" />
              <strong>{profile.character.name}</strong>
            </>
          )}
          <span className="dataVersion">
            <Database size={13} />
            {status.version ? `Dados ${status.version}` : 'Sem dados sincronizados'}
          </span>
        </div>

        <section
          className={page === 'teams' || page === 'builds' ? 'content contentWorkspace' : 'content'}
        >
          {error && (
          ...
          {page === 'characters' ? (
              ...
              <>
                <div className="pageIntro">
                  <div>
                    ...
                    <h1>PERSONAGENS</h1>
                    <p>
                      Explore o elenco, filtre funções e organize quem já faz parte da sua conta.
                    </p>
                  </div>
                  <div className="versionCard">
                    ...
                    <small>
                      {status.lastSyncAt
                        ? `Atualizado ${formatDate(status.lastSyncAt)}`
                        : 'Aguardando primeira sincronização'}
                    </small>
                    {status.count > 0 && (
                      <button onClick={() => void restore()}>Restaurar snapshot</button>
                    )}
                  </div>
                </div>
                ...
                <div className="toolbar">
                  ...
                  <select
                    ...
                    value={filter.sort}
                    onChange={(event) =>
                      setFilter({ ...filter, sort: event.target.value as CharacterFilter['sort'] })
                    }
                  >
                ...
                <div className="filterRow">
                  <div className="chips" aria-label="Filtrar por elemento">
                    {elements.map(([code, name]) => (
                      ...
                      >
                        {code > 0 && (
                          <i className={`elementDot element-${code}`} aria-hidden="true" />
                        )}
                        {name}
                      </button>
                    ))}
                  ...
                  <label className="check">
                    <input
                      ...
                      onChange={(event) =>
                        setFilter({ ...filter, ownedOnly: event.target.checked })
                      }
                    />
                    Somente possuídos
                  </label>
                  <label className="check">
                    <input
                      ...
                      onChange={(event) =>
                        setFilter({ ...filter, favorites: event.target.checked })
                      }
                    />
                    Favoritos
                  </label>
            ...
            <AssistantPage onError={setError} />
          ) : page === 'settings' ? (
            <SettingsPage onError={setError} />
          ) : (
            <ComingSoon page={page} onNavigate={setPage} />
          )}
        </section>
      </main>
    </div>
...
function CharacterGrid({
  ...
  onFavorite: (character: Character) => Promise<void>;
}) {
  ...
  return (
    <>
        ...
        {items.map((item) =>
          item.kind === 'rover' ? (
            <RoverGroupCard
              ...
              onOpen={() => setShowRoverPicker(true)}
            />
          ) : (
            <article
            ...
            >
              <div className={`portrait element-${item.character.elementCode}`}>
                {item.character.iconPath.startsWith('/cache/') && (
                  <img
                    src={item.character.iconPath}
                    alt=""
                    loading="lazy"
                    onError={(event) => {
              ...
              <div className="cardBody">
                <div className="cardTitle">
                  <div>
                    ...
                    {item.character.nickname && item.character.nickname !== item.character.name && (
                      <p>{item.character.nickname}</p>
                    )}
                  </div>
                  <button
                    className="favorite"
                    onClick={(event) => {
                      ...
                      void onFavorite(item.character).catch(() => undefined);
                    }}
                    aria-label={`Favoritar ${item.character.name}`}
                  >
                    <Star
                      size={17}
                      strokeWidth={1.5}
                      fill={item.character.favorite ? 'currentColor' : 'none'}
                    />
                  </button>
                </div>
                <div className="tags">
                  <span>
                    <i className={`elementDot element-${item.character.elementCode}`} />
                ...
                <footer>
                  <span>
                    {item.character.owned
                      ? `NV. ${item.character.level} · S${item.character.sequence}`
                      : 'NÃO REGISTRADO'}
                  </span>
      ...
      {showRoverPicker && (
        <RoverVariantPicker
          variants={rover}
          onClose={() => setShowRoverPicker(false)}
          onOpen={(id) => {
      ...
      <div className="tableWrap">
        <table>
          ...
          <tbody>
            {items.map((item) =>
              item.kind === 'rover' ? (
                <tr key="rover-group" className="roverTableRow">
                  <td>
                    <span className="miniPortrait roverMiniPortrait">R</span>
                    <span>
                      <strong>Rover</strong>
                      <small>{item.variants.length} variantes oficiais</small>
                    </span>
                  </td>
                  <td>
                    <span className="tableTag">4 ELEMENTOS</span>
                  </td>
                  <td>
                    <RarityStars value={5} />
                  </td>
                  <td>Sword</td>
                  <td>{item.variants.filter((character) => character.owned).length} registradas</td>
                  <td>
                    <button
                      onClick={() => setShowRoverPicker(true)}
                      aria-label="Escolher variante do Rover"
                    >
                ...
                <tr key={item.character.id}>
                  <td>
                    <span className={`miniPortrait element-${item.character.elementCode}`}>
                      {initials(item.character.name)}
                    </span>
                    <span>
                      <strong>{item.character.name}</strong>
                      <small>{item.character.nickname}</small>
                    </span>
                  </td>
                  <td>
                    <span className="tableTag">
                      <i className={`elementDot element-${item.character.elementCode}`} />
                      {item.character.element}
                    </span>
                  </td>
                  ...
                  <td>
                    <button
                      onClick={() => onOpen(item.character.id)}
                      aria-label={`Abrir ${item.character.name}`}
                    >
                      <ChevronRight size={18} />
                    </button>
                  </td>
...
type CharacterCatalogItem =
  { kind: 'character'; character: Character } | { kind: 'rover'; variants: Character[] };
...
function groupCharacterCatalog(characters: Character[]): CharacterCatalogItem[] {
  ...
  return characters.flatMap((character) => {
    if (!isRoverCharacter(character))
      return [{ kind: 'character', character } as CharacterCatalogItem];
    if (added) return [];
    added = true;
...
function RoverGroupCard({ variants, onOpen }: { variants: Character[]; onOpen: () => void }) {
  ...
  const elements = Array.from(
    new Map(variants.map((character) => [character.elementCode, character])).values()
  );
  return (
    <article
      className="characterCard roverGroupCard"
      role="button"
      tabIndex={0}
      onClick={onOpen}
      onKeyDown={(event) => {
    ...
    >
      <div className="roverPortraitPair">
        {[female, male].map((character, index) => (
          <div key={character?.id ?? index} className={`element-${character?.elementCode ?? 0}`}>
            {character?.iconPath.startsWith('/cache/') && (
              <img src={character.iconPath} alt="" loading="lazy" />
            )}
          </div>
        ))}
        <span className="roverMark">R</span>
      </div>
      <div className="cardBody">
        <div className="cardTitle">
          <div>
            <h2>Rover</h2>
            <p>Masculino e feminino · {variants.length} variantes</p>
          </div>
        </div>
        <div className="roverElementList">
          {elements.map((character) => (
            <span key={character.elementCode}>
              <i className={`elementDot element-${character.elementCode}`} />
              {character.element}
            </span>
          ))}
        </div>
        <footer>
          <span>ESCOLHER GÊNERO E ELEMENTO</span>
...
function RoverVariantPicker({
  ...
  const groups = (['female', 'male'] as const)
    .map((gender) => ({
      gender,
      characters: variants
        .filter((character) => roverGender(character) === gender)
        .sort((a, b) => a.apiOrder - b.apiOrder),
    }))
    .filter((group) => group.characters.length);
  return (
    <div className="modalBackdrop roverPickerBackdrop" onMouseDown={onClose}>
      <section
        className="roverPicker"
        role="dialog"
        aria-modal="true"
        aria-labelledby="rover-picker-title"
        onMouseDown={(event) => event.stopPropagation()}
      >
        <header>
          <div>
            <span className="sectionLabel">PERSONAGEM MULTIFORMA</span>
            <h2 id="rover-picker-title">Escolha o Rover</h2>
            <p>Gênero e elemento mantêm IDs, progressão e habilidades independentes.</p>
          </div>
          <button onClick={onClose} aria-label="Fechar">
            <X size={19} />
          </button>
        </header>
        <div className="roverGenderGroups">
          {groups.map((group) => (
            <section key={group.gender}>
              <div className="roverGenderHeading">
                <UserRound size={17} />
                <div>
                  <b>{group.gender === 'female' ? 'Feminino' : 'Masculino'}</b>
                  <small>{group.characters.length} elementos</small>
                </div>
              </div>
              <div className="roverVariants">
                {group.characters.map((character) => (
                  <button
                    key={character.id}
                    className={`element-${character.elementCode}`}
                  ...
                  >
                    ...
                    <span>
                      ...
                      <small>
                        {character.owned
                          ? `Nv. ${character.level} · S${character.sequence}`
                          : 'Não registrado'}
                      </small>
                    </span>
...
function EmptyState({
  ...
  return (
    <div className="emptyState">
      ...
      <p>
        {hasCatalog
          ? 'Ajuste ou limpe os filtros para voltar a explorar o catálogo.'
          : 'Faça a primeira sincronização para baixar o índice oficial e usá-lo offline.'}
      </p>
      {!hasCatalog && (
        <button className="primaryButton" onClick={onSync} disabled={syncing}>
          {syncing ? 'Sincronizando…' : 'Sincronizar personagens'}
        </button>
      )}
    </div>
  );
...
function Loading() {
  return (
    <div className="skeletonGrid" aria-label="Carregando personagens">
...
function ComingSoon({ page, onNavigate }: { page: PageID; onNavigate: (page: PageID) => void }) {
  ...
  return (
    <div className="comingSoon">
      ...
      <p>
        {priorities[page] ??
          'Este módulo faz parte do roadmap e ainda não foi conectado à fundação atual.'}
      </p>
      <div>
        <button className="primaryButton" onClick={() => onNavigate('characters')}>
          Abrir personagens
        </button>
        <button onClick={() => onNavigate('weapons')}>Abrir armas</button>
      </div>
    </div>
  );
}
...
function RarityStars({ value }: { value: number }) {
  return (
    <span className="rarityIcons" aria-label={`${value} estrelas`}>
      {Array.from({ length: value }, (_, index) => (
        <Star key={index} size={9} fill="currentColor" strokeWidth={1.5} />
      ))}
    </span>
  );
}
